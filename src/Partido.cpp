...
void Partido::actualizarHistoricos() {
    ...
    for (int i=0; i<convL.getTam(); i++)
        convL[i].getJugador()->actualizarStats(convL[i].toStats());
    Lista<ActaJugador>& convV = actaVisitante.getConvocados();
    for (int i=0; i<convV.getTam(); i++)
        convV[i].getJugador()->actualizarStats(convV[i].toStats());
    simulado = true;
}

/// Imprime numero de camiseta de los goleadores de cada equipo
void Partido::imprimirGoleadores() const {
    ...
    for (int i=0; i<cL.getTam(); i++)
        if (cL[i].getGoles()>0)
            std::cout << "#" << cL[i].getJugador()->getNumeroCamiseta()
                      << "(" << cL[i].getGoles() << ") ";
    ...
    for (int i=0; i<cV.getTam(); i++)
        if (cV[i].getGoles()>0)
            std::cout << "#" << cV[i].getJugador()->getNumeroCamiseta()
                      << "(" << cV[i].getGoles() << ") ";
    std::cout << "\n";
}
